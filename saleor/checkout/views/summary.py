...
def _handle_order_placement(request, checkout):
    ...
    try:
        ...
        messages.warning(
            request, pgettext("Checkout warning",
                              "Please review your checkout.")
        )
        ...
        messages.warning(
            request,
            pgettext(
                "Checkout warning", "Unable to calculate taxes - %s" % str(
                    tax_error)
            ),
        )
        return redirect("checkout:summary")
    ...
    checkout.delete()
    messages.success(request, pgettext('checkout success' ,"Your Order was made successfully!"),)
